...
f32 Game_RandomInRange(f32 range)
{
    return COM_STDRandf32() * (range - -range) + -range;
}

f32 DotProduct2D(f32 vx0, f32 vy0, f32 vx1, f32 vy1)
{
    return vx0 * vx1 + vy0 * vy1;
}


u8 IsPointLeftOfLine(f32 lineX, f32 lineY, f32 lineVX, f32 lineVY, f32 pX, f32 pY)
{
    f32 vx = lineX - pX;
    f32 vy = lineY - pY;
    f32 normalX = lineVY;
...
Vec3 Vec3_ForwardFromAngles(f32 yawDegrees, f32 pitchDegrees, f32 magnitude)
{
    Transform t;
...
EntId AI_FindNearestPlayer(ClientList* clients, GameScene* scene, Vec3 pos)
{
    EntId result = {};
	Client* cl = NULL;
...
void Ent_SetProjectileSpawnOptions(
