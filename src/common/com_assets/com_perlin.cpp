...
static f32 lin_inter(f32 x, f32 y, f32 s)
{
    return x + s * (y-x);
}

static f32 smooth_inter(f32 x, f32 y, f32 s)
{
...
static f32 noise2d(f32 x, f32 y)
{
    ...
    const f32  low = smooth_inter( (f32)s, (f32)t, x_frac );
    const f32  high = smooth_inter( (f32)u, (f32)v, x_frac );
    const f32  result = smooth_inter( low, high, y_frac );
    return result;
}

f32 Perlin_Get2d(f32 x, f32 y, f32 freq, int depth)
{
    f32  xa = x*freq;
    f32  ya = y*freq;
    f32  amp = 1.0;
