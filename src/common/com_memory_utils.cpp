 ...
 */
com_internal inline u32 COM_WriteByte(u8 value, u8* target)
{
	Assert(target != NULL);
    *target = value;
    return sizeof(u8);
...
com_internal inline void COM_WriteByte(u8 value, u8** target)
{
	Assert(target != NULL);
    **target = value;
    *target++;
 ...
 * Returns number of bytes written
 */
com_internal inline u32 COM_WriteI16(i16 value, u8* target)
{
	Assert(target != NULL);
    *(i16*)target = value;
    return sizeof(u16);
 ...
 * Returns number of bytes written
 */
com_internal inline u32 COM_WriteU16(u16 value, u8* target)
{
	Assert(target != NULL);
    *(u16*)target = value;
    return sizeof(u16);
 ...
 * Returns number of bytes written
 */
com_internal inline u32 COM_WriteI32(i32 value, u8* target)
{
	Assert(target != NULL);
    *(i32*)target = value;
    return sizeof(i32);
 ...
 * Returns number of bytes written
 */
com_internal inline u32 COM_WriteU32(u32 value, u8* target)
{
	Assert(target != NULL);
    *(u32*)target = value;
    return sizeof(u32);
 ...
 * Returns number of bytes written
 */
com_internal inline u32 COM_WriteF32(f32 value, u8* target)
{
	Assert(target != NULL);
    *(f32*)target = value;
    return sizeof(f32);
 ...
 */
com_internal inline u8 COM_ReadByte(u8** target)
{
	Assert(target != NULL);
    u8 result = *(u8*)*target;
    *target += sizeof(u8);
...
com_internal inline u16 COM_ReadU16(u8** target)
{
	Assert(target != NULL);
 ...
 * Read an i32 at the target pointer position AND move the target pointer forward
 */
com_internal inline i32 COM_ReadI32(u8** target)
{
	Assert(target != NULL);
...
com_internal inline u32 COM_ReadU32(u8** target)
{
	Assert(target != NULL);
...
com_internal inline f32 COM_ReadF32(u8** target)
{
	Assert(target != NULL);
 ...
 */
com_internal inline i32 COM_PeekI32(u8* target)
{
	Assert(target != NULL);
    i32 result = *(i32*)target;
    return sizeof(i32);
...
com_internal inline i32 COM_CompareMemory(u8* ptrA, u8* ptrB, u32 numBytes)
{
...
com_internal inline u32 COM_GetI32Sentinel()
{
	u32_union u;
	u.bytes[0] = 'D';
	u.bytes[1] = 'E';
...
com_internal inline i32 COM_SimpleHash(u8* ptr, i32 numBytes)
{
    #if 0
